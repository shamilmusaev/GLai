  ...
  return (
    <nav className="md-:py6 px-4 py-4 md:px-6" aria-label="Main">
      <div className="  mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-white md:flex-row md:items-center">
        ...
        <div
          className={clsx(
            "item-end fixed bottom-0 left-0 right-0 top-0 z-40 flex flex-col gap-4 bg-[#070815] pr-4 pt-14 transition-transform duration-300 ease-in-out motion-reduce:transition-none md:hidden",
        ...
        <ul className="flex hidden gap-6 md:flex">
          {settings.data.navigation.map((item) => {
            if (item.cta_button) {
              return (
                <li key={item.label}>
